  ...
  (event) => (dispatch, getState) => {
    ...
    switch (reason) {
      case 'connection error':
        if (!willReconnect) {
          dispatch(showError('Error while connecting to MIT server'));
        }
      ...
      case 'connection timeout':
        if (!willReconnect) {
          dispatch(showError('Timeout while connecting to MIT server'));
        }

        break;

      case 'io client disconnect':
        dispatch(showNotification('Disconnected from MIT server'));
        break;
      ...
      case 'transport close':
        dispatch(showError('MIT server closed connection unexpectedly'));
        break;

      case 'ping timeout':
        dispatch(showError('Connection to MIT server lost'));
        break;
  ...
  (dispatch) => ({
    ...
    onLocalServerError(message, wasRunning) {
      const baseMessage = wasRunning
        ? 'MIT server died unexpectedly'
        : 'Failed to launch local MIT server';
      dispatch(setCurrentServerConnectionState(ConnectionState.DISCONNECTED));
