    ...
    case Source.OSM:
      return (
        <source.OSM
          ...
          // url='https://127.0.0.1:443/tile/skybrush/{z}/{x}/{y}'
        />
      );
 ...
 * server caches.
 */
function getCachedTileLoader(serverUrl, type) {
  return (imageTile, url) => {
    ...
    switch (type) {
      ...
      case Source.MAPBOX.SATELLITE:
      case Source.MAPBOX.VECTOR:
        api_key = newUrl.searchParams.get('access_token');
        break;
      case Source.MAPTILER.BASIC:
      case Source.MAPTILER.HYBRID:
      case Source.MAPTILER.SATELLITE:
      case Source.MAPTILER.STREETS:
        api_key = newUrl.searchParams.get('key');
        break;
    }
    const cachedUrl = `${serverUrl}/map_cache/${coords[0]}/${coords[1]}/${coords[2]}.png?type=${type}&apiKey=${api_key}`;
...
const getMapTileLoaderFunction = createSelector(
  ...
  getBaseLayerSourceType,
  // getAPIKeys,
  (cachingEnabled, cachingSupported, serverUrl, type) =>
    cachingEnabled && cachingSupported
      ? getCachedTileLoader(serverUrl, type)
      : loadTile
);
