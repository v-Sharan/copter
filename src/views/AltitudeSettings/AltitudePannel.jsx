...
import {
  changeAltitude,
  changeDifference,
  changeAltitudesArray,
  changeSingeAltitude,
  changeInitial,
  changeData,
  changeInitialDataTonule,
} from '~/features/altitudeSettings/slice';
...
const AltitudeSettings = ({ ids, isHaveUavs, dispatch, altitudeSettings }) => {
  ...
  React.useEffect(() => {
    ...
    const newAltitudes = Array.from(
      { length: ids.length },
      (_, i) => altitudeSettings.altitude + i * altitudeSettings.difference
    );
    ...
    try {
      ...
      dispatch(
        showError(
          e?.message
            ? e.message
            : `Something went wrong in Changing Altitude of ${sysid}`
        )
      );
    }
  };
  ...
  const calculateAltitudes = async () => {
    ...
    try {
      ...
      dispatch(
        showError('Something went wrong while trying to send altitudes.')
      );
    }
  };
        ...
        <FormControl variant='standard'>
          ...
          <Input
            ...
            onChange={({ target: { value } }) =>
              dispatch(changeAltitude({ altitude: parseInt(value) }))
            }
          />
        </FormControl>
        <FormControl variant='standard'>
          ...
          <Input
            ...
            onChange={({ target: { value } }) =>
              dispatch(changeDifference({ difference: parseInt(value) }))
            }
          />
        </FormControl>
        <Button variant={'contained'} onClick={calculateAltitudes}>
          Save
        </Button>
      </FormGroup>
      {isHaveUavs &&
        ids.map((id, i) => (
          <div key={id} className={classes.wrapper}>
            ...
            <Input
              ...
              onChange={({ target: { value } }) =>
                dispatch(
                  changeSingeAltitude({ index: i, value: parseInt(value) })
                )
              }
            />
            <Button
              variant='contained'
              onClick={async () =>
                handleSendAltChange(id, altitudeSettings.altitudes[i])
              }
            >
              Change Altitude
            </Button>
