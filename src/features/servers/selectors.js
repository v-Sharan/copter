...
export const getMapCacheUrl = createSelector(
  (state) => state.dialogs.serverSettings,
  ({ hostName, port, isSecure }) => {
    const protocol = isSecure ? 'https:' : 'http:';
    return hostName ? `${protocol}//${hostName}:5000` : undefined;
  }
);

export const getBaseLayerSourceType = (state) =>
  state.map.layers.byId['base'].parameters.source;
